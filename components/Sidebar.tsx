...
import { Home, AlertTriangle, FileText, Settings, BarChart3, RefreshCw, LogOut, Store, CreditCard, PlusCircle, ChevronDown, ChevronRight } from 'lucide-react';
    ...
    <div className={clsx(
      ...
      <nav className={clsx("flex-1 space-y-1 py-6", isCollapsed ? "px-2" : "px-4")}>
        {navItems.map((item) => {
          ...
          const isActive = pathname === item.href || (item.href !== '/dashboard' && pathname?.startsWith(`${item.href}/`)) || (item.name === 'Dashboard' && pathname === '/dashboard');
          ...
          return (
            <div key={item.name}>
              ...
              >
                ...
                {!isCollapsed && hasChildren && (
                  <button
                    ...
                    className="p-1 rounded-md hover:bg-slate-200 dark:hover:bg-neutral-800 transition-colors"
                  >
              ...
              {!isCollapsed && hasChildren && isExpanded && (
                <div className="ml-9 mt-1 space-y-1 border-l border-slate-200 dark:border-neutral-800 pl-2">
