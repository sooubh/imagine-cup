...
export async function seedFreshData() {
    try {
        ...
        // Seed FDC (Food Distribution Center) data - 50 items × 3 stores = 150
        ...
        for (let storeIndex = 0; storeIndex < storesBySection.FDC.length; storeIndex++) {
            ...
            console.log(`  Adding 50 items for store: ${storeId}`);

            for (const item of FDC_SEED_DATA) {
        ...
        // Seed Hospital data - 50 items × 4 stores = 200
        ...
        for (let storeIndex = 0; storeIndex < storesBySection.Hospital.length; storeIndex++) {
            ...
            for (const item of FDC_SEED_DATA) { // Reusing FDC items as template
                await azureService.addItem({
                    ...generateVariation(item, storeIndex),
        ...
        console.log(`✅ Added ${storesBySection.Hospital.length * FDC_SEED_DATA.length} items to Hospital\n`);

        // Seed NGO data - 50 items × 4 stores = 200
        ...
        console.log(`✅ Added ${storesBySection.NGO.length * FDC_SEED_DATA.length} items to NGO\n`);
        ...
        return {
            success: true,
            message: `Successfully seeded ${totalItems} items to retailer stores (50 items × 3 stores × 3 sections). Admins can now manage these stores.`,
            stats: {
                deleted: deletedCount,
                FDC: storesBySection.FDC.length * FDC_SEED_DATA.length,
                Hospital: storesBySection.Hospital.length * FDC_SEED_DATA.length,
                NGO: storesBySection.NGO.length * FDC_SEED_DATA.length,
                total: totalItems
            }
        };
