...
export function StockHeatmapTable({ limit, items }: StockHeatmapTableProps) {
  ...
  const { processedData, locations, hasMore } = useMemo(() => {
    ...
    const itemDetails = new Map<string, { id: string, category: string, unit: string, section: string }>();
    ...
    filtered.forEach(item => {
      ...
      if (!pivot.has(itemName)) {
        ...
        itemDetails.set(itemName, { id: item.id, category: item.category, unit: "units", section: item.section });
      }
    ...
    rows.sort((a, b) => {
      if (a.details.category < b.details.category) return -1;
      if (a.details.category > b.details.category) return 1;
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody className="divide-y divide-neutral-50 dark:divide-neutral-800/50 text-sm">
            {processedData.length > 0 ? (
              processedData.reduce((acc: React.ReactNode[], row, index, array) => {
                const prevRow = index > 0 ? array[index - 1] : null;
                const isNewCategory = !prevRow || prevRow.details.category !== row.details.category;

                if (isNewCategory) {
                  acc.push(
                    <tr key={`header-${row.details.category}`} className="bg-neutral-50 dark:bg-neutral-800/50">
                      <td colSpan={locations.length + 5} className="px-6 py-2 text-xs font-bold uppercase tracking-wider text-neutral-500 dark:text-neutral-400">
