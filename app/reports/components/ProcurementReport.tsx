...
export function ProcurementReport({ orders, isLoading }: ProcurementReportProps) {
    ...
    const receivedOrders = orders.filter(o => o.status === 'COMPLETED').length;
                            ...
                            {orders.map(o => (
                                <tr key={o.id} className="hover:bg-neutral-50 dark:hover:bg-white/5">
                                    ...
                                    <td className="py-3 px-4">
                                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${o.status === 'COMPLETED' ? 'bg-green-100 text-green-700' :
